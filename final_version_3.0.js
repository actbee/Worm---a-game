...
class garbage{      // a single ball,eat it to get point  小球修改
 ...
 //   fill(150,150,150,this.lifespan);
  ...
  hit(){
    ...
    for(let i=0;i<mymonster.monsters.length;i++){
      if(mymonster.monsters[i].askfresh()==true){
        continue;
      }
...
class explodeball{            //part of the exploding effect
  constructor(inx,iny){
     this.v=createVector(random(-0.8*scale,0.8*scale),random(-0.8*scale,0.8*scale));
     this.lifespan=255;
  ...
  constructor(){
   ...
   update(){
    ...
    this.v.add(this.a);
     ...
     if(gamemode==1){
      this.location=createVector(myhand.loc.x,myhand.loc.y);
     }
     else if(gamemode==2){
      let dis=p5.Vector.sub(myhand.loc,this.location);
...
class monstersystem{  //control all the monsters
  ...
  more(){
    ...
    for(let i=0;i<addnum;i++){
      let decide=random(0,1);    //the way to create monsters
      if(decide<addnum/5){
      this.monsters.push(new monster_move());
      this.num++;
      }  
      else{
      this.monsters.push(new monster_static());
...
class monster_base{  //the father of all monsters
  constructor(value_a){
    ...
    this.addmax=random(30,80)*scale;
    this.vmax=scale;
    this.freshlife=0;
    this.isfresh=true;
  }
  update(){
     this.v.add(this.a);
     if(this.v.mag()>this.vmax){
       this.v.setMag(this.vmax);
     }
     this.add_loc.add(this.v);
     if(this.add_loc.mag()>this.addmax){
       this.a.x=-this.a.x;
       this.a.y=-this.a.y;
       this.v.x=-this.v.x;
       this.v.y=-this.v.y;
...
class monster_static extends monster_base{ // basic static monster 方块绘制
  ...
  render(){
    ...
    if(this.isfresh==true){
      strokeWeight(scale*2*(this.freshlife/250));
      stroke(130+110*sin(millis()/1000),0,0);
      fill(225+25*sin(millis()/1000),this.freshlife);
    }
    else{
...
class monster_move extends monster_base{ // basic move monster 方块绘制
  constructor(){
    super(createVector(random(-0.5*scale,0.5*scale),random(-0.5*scale,0.5*scale)));
    this.sizex=15*scale;
   // this.sizey=random(10,40);
    this.sizey=this.sizex;
    this.tag=2;
...
class hand{
  constructor(){
    this.loc=createVector(trackloc.x,trackloc.y);
    this.v=createVector(0,0);
  ...
  update(){
  /* this.a=p5.Vector.sub(trackloc,this.loc);
   if(this.a.mag()>this.amax){
        this.a.setMag(this.amax);
   }
   this.v.add(this.a);*/
   this.v=p5.Vector.sub(trackloc,this.loc);
   if(this.v.mag()>this.vmax){
     this.v.setMag(this.vmax);
   }
   this.loc.add(this.v);
 ...
 draw(){
   ...
   if(stop==false){
    ...
    ellipse(this.loc.x+random(-15,15),this.loc.y+random(-15,15),r,r);
    }
    pop();
   }
 }
