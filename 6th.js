...
class garbagesystem{    //the ball system, manage all the balls
  constructor(){
    this.max=20;
    this.num=Math.floor(random(10,20));
    this.garbages=[];
...
class fieldpoint{  // single point,part of the field 修改网格扩张方式
  constructor(inx,iny){
    ...
    this.add_loc=createVector(0,0);
    this.limit=20*scale;   //max span distance 最大扩张距离
  ...
  field(){
    ...
    push();
     /* colorMode(HSB,360,255,100);
      stroke(100+100*sin(millis()/1000),255,100);  // color of network 网格颜色
      */
      stroke(80,80,80);
      strokeWeight(1.2*scale);  // weight of network 网格粗细
      for(let i=0;i<this.xnum;i++){
        for(let j=0;j<this.ynum;j++){
          if(i+1<this.xnum){
