...
class garbage{      // a single ball,eat it to get point  小球修改
  ...
  render(){   //way to draw garbage 小球绘制方式
    ...
    //  strokeWeight(this.freshlife*this.r*this.changev/2000);   //stroke of garbage 小球边的绘制，后同
      ...
      fill(this.colorbeg+100*sin(millis()/1000),105,100,this.freshlife); 
  //    fill(150,150,150,this.freshlife); //color of garbage 小球颜色的绘制，后同
    }
    else{
    strokeWeight(0);
    colorMode(HSB,360,255,100,this.lifespan);
    fill(this.colorbeg+100*sin(millis()/1000),105,100); 
...
class garbagesystem{    //the ball system, manage all the balls
  constructor(){
    this.max=7*scale;
    this.num=Math.floor(random(3*scale,7*scale));
    this.garbages=[];
...
class fieldsystem{  // the background net 修改网格
  ...
  field(){
    ...
    //  stroke(80,80,80);
      strokeWeight(0.2*scale);  // weight of network 网格粗细
      for(let i=0;i<this.xnum;i++){
        for(let j=0;j<this.ynum;j++){
          if(i+1<this.xnum){
   ...
   update(){
    ...
    this.v.add(this.a);
     ...
     /* let dis=p5.Vector.sub(trackloc,this.location);
      if(dis.mag()>this.segNum*this.segLength/2){
        this.location=createVector(trackloc.x+random(-40*scale,40*scale),trackloc.y+random(-40*scale,40*scale));
      }
      else{
      this.v=createVector(dis.x,dis.y);
      this.v.setMag(random(0.2*scale,0.35*scale));
      this.v.add(this.a);
      this.location.add(this.v);
      }*/
    }
...
function draw() {
  ...
  background(change_color,off);
  change_color+=coloroff;
  if(change_color>50||change_color<20){
       coloroff*=-1;
  }
  //noisearea();
  randomlines();
